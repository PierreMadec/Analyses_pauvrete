...
g2 <- resultats |>
  ...
  mutate(effet = factor(effet,
    levels = c("effet_seuil", "effet_structure_typmen", "effet_structure_acteu",
               "effet_redistribution", "effet_rev_primaires"),
    labels = c("Seuil (médian)", "Structure : type de ménage",
               "Structure : activité", "Redistribution", "Revenus primaires")),
  ...
  labs(
    ...
    caption = "Source : INSEE, ERFS 2005-2023, calculs OFCE. La ligne noire représente la variation totale du taux de pauvreté."
  ) +
  theme_minimal(base_size = 13) +
  theme(
